...
namespace antivirus_clamav\local\hooks\navigation;

use admin_setting_configexecutable;
use admin_setting_configselect;
use admin_setting_configtext;
use antivirus_clamav_pathtounixsocket_setting;
use antivirus_clamav_runningmethod_setting;
use antivirus_clamav_tcpsockethost_setting;
use lang_string;

/**
 * Hook callbacks for antivirus_clamav
 *
 * @package    antivirus_clamav
 * @copyright  2023 Marina Glancy
 * @author     2015 Ruslan Kabalin, Lancaster University.
...
class site_administration_extend {
    ...
    public static function callback(\core\hook\navigation\site_administration_extend $hook): void {
        ...
        $settings = $hook->create_settingpage_for_plugin(explode('\\', __CLASS__)[0]);

        if ($ADMIN->fulltree) {
            require_once($CFG->dirroot . '/lib/antivirus/clamav/adminlib.php');
            require_once($CFG->dirroot . '/lib/antivirus/clamav/classes/scanner.php');
