...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        initializer: function(options) {
            ...
            this.filepickerqup_options = {}
            for (var p in this.filepicker_options) {
                if (this.filepicker_options.hasOwnProperty(p)) {
                    this.filepickerqup_options[p] = this.filepicker_options[p]
                }
            }
            this.filepicker_options.quickupload = false;
            this.filepickerqup_options.quickupload = true;
            this.filepickerqup_options.client_id = 'qup' + this.filepickerqup_options.client_id;
        ...
        check_buttons: function() {
            ...
            var button_upload   = Y.one("#btnqup-"+this.client_id);
            if (this.filecount > 0) {
                button_download.setStyle('display', 'inline-block');
            } else {
                button_download.setStyle('display', 'none');
            }
            if (this.filecount >= this.maxfiles && this.maxfiles!=-1) {
                button_addfile.setStyle('display', 'none');
                button_upload.setStyle('display', 'none');
            } else {
                button_addfile.setStyle('display', 'inline-block');
                button_upload.setStyle('display', 'inline-block');
            }
        },
        ...
        setup_buttons: function() {
            ...
            var button_addfile  = Y.one("#btnadd-"+this.client_id);
            var button_qupload  = Y.one("#btnqup-"+this.client_id);

            // setup 'Add...' and 'Quick upload' buttons
            button_addfile.on('click', function(e) {
                this.filepicker_options.formcallback = this.filepicker_callback;
                // XXX: magic here, to let filepicker use filemanager scope
                this.filepicker_options.magicscope = this;
                this.filepicker_options.savepath = this.currentpath;
                M.core_filepicker.show(Y, this.filepicker_options);
            }, this);
            button_qupload.on('mouseover', function(e) {
                this.filepickerqup_options.formcallback = this.filepicker_callback;
                // XXX: magic here, to let filepicker use filemanager scope
                this.filepickerqup_options.magicscope = this;
                this.filepickerqup_options.savepath = this.currentpath;
                M.core_filepicker.show(Y, this.filepickerqup_options);
            }, this);
