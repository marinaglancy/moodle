        ...
        setup_select_file: function() {
            ...
            selectnode.one('.fp-file-unzip').on('click', function(e) {
                ...
                this.request({
                    ...
                    params: params,
                    callback: function(id, obj, args) {
                        if (obj.unknown_encoding) {
                            var scope = args.scope;
                            // a function used to perform an ajax request 
                            var unzip_update_preview = function(e) {
                                e.preventDefault();
                                Y.one('.fp-unzipencoding-dlg-preview').addClass('loading');
                                var params = {};
                                params['filepath'] = fileinfo.filepath;
                                params['filename'] = fileinfo.fullname;
                                params['encoding'] = Y.one('#fm-unzip-encoding-'+scope.client_id).get('value');
                                params['preview'] = true;
                                scope.request({
                                    action: 'unzip',
                                    scope: scope,
                                    params: params,
                                    callback: function(id, obj, args) {
                                        Y.one('.fp-unzipencoding-dlg-preview').removeClass('loading');
                                        Y.one('.fp-unzipencoding-dlg-preview').replace(obj.preview);
                                    }
                                });
                            }
                            // a function used to perform an ajax request
                            var unzip_with_encoding = function(e) {
                                e.preventDefault();
                                var params = {};
                                params['filepath'] = fileinfo.filepath;
                                params['filename'] = fileinfo.fullname;
                                params['encoding'] = Y.one('#fm-unzip-encoding-'+scope.client_id).get('value');
                                scope.selectui.show();
                                args.scope.unzip_encoding_dialog.hide();
                                scope.request({
                                    action: 'unzip',
                                    scope: scope,
                                    params: params,
                            ...
                            if (!scope.unzip_encoding_dialog) {
                                var node = Y.Node.createWithFilesSkin(M.form_filemanager.templates.unzipencoding);
                                scope.unzip_encoding_dialog = new Y.Panel({
                                    srcNode      : node,
                                    zIndex       : 800000,
                                    centered     : true,
                                    modal        : true,
                                ...
                                scope.unzip_encoding_dialog.plug(Y.Plugin.Drag,{handles:['.yui3-widget-hd']});
                                node.one('.fp-unzipencoding-dlg-select').replace(obj.select_list);
                                node.one('.fp-unzipencoding-dlg-select').set('id', 'fm-unzip-encoding-'+scope.client_id);
                                node.one('.fp-unzipencoding-dlg-select').on('change', unzip_update_preview, this);
                                node.one('.fp-unzipencoding-dlg-preview').replace(obj.preview);
                                node.one('.fp-dlg-butunzip').on('click', unzip_with_encoding, this);
                                node.one('.fp-dlg-butcancel').on('click', 
                                    function(e) {
                                        e.preventDefault();
                                        scope.unzip_encoding_dialog.hide();
                                    }, this);
                            }
                            scope.selectui.hide();
                            scope.unzip_encoding_dialog.show();
                        } else {
                            args.scope.selectui.hide();
                            args.scope.refresh(obj.filepath);
                        }
