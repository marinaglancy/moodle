...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        setup_buttons: function() {
            ...
            this.filemanager.one('.fp-mm-switch').on('click', function(e) {
                e.preventDefault();
                this.filemanager.toggleClass('fm-multimode');
            }, this);

            this.filemanager.one('.fp-mm-action').on('click', function(e) {
                e.preventDefault();
                if (!this.filemanager.hasClass('fm-multimode')) {
                    return;
                }
                this.select_multiple_files();
            }, this);
        },
            ...
            var options = {
                ...
                callback : function(e, node) {
                    ...
                    if (this.filemanager.hasClass('fm-multimode')) {
                        ...
                        this.filemanager.one('[data-filepath="'+node.filepath.replace(/[\\"']/g, '\\$&')+
                                '"][data-filename="'+node.fullname.replace(/[\\"']/g, '\\$&')+'"]').
                                addClassIf('fp-checked', node.checked);
                ...
                classnamecallback : function(node) {
                    ...
                    if (node.checked) {
                        classname = classname + ' fp-checked';
                    }
        ...
        select_multiple_files: function() {
            var paths = [], hasfolders = false;
            this.filemanager.all('[data-filename].fp-checked').each(function(el) {
                paths.push({
                    filepath : el.getAttribute('data-filepath'),
                    fullname : el.getAttribute('data-filename'),
                    filename : el.hasClass('fp-folder')?'.':el.getAttribute('data-filename')
                });
                hasfolders = hasfolders || el.hasClass('fp-folder');
            });
            console.log(paths);
        },
        render: function() {
