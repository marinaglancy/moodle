 ...
 *
 * Example of usage in javascript - display form in a modal dialogue:
 *
 ...
 * });
 *
 * Example of usage in javascript - load and save the form in AJAX requests without reloading page.
 *
 * <a href="#" data-action="loadform">Load</a>
 * <div id="container"></div>
 *
 * require(['jquery', 'core_form/ajaxform'], function($, AjaxForm) {
 *     $('[data-action=loadform]').on('click', function(e) {
 *         e.preventDefault();
 *         form = new AjaxForm($('#container')), 'pluginname\\formname', {});
 *         // If necessary extend functionality by overriding class methods, for example:
 *         form.onSubmitSuccess = function(response) {
 *             this.container.html('<pre>'+JSON.stringify(response)+'</pre>');
 *         }
 *     });
 * });
 *
