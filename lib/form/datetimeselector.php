...
class MoodleQuickForm_date_time_selector extends MoodleQuickForm_group {
    ...
    function _createElements() {
        ...
        foreach ($dateformat as $key => $date) {
            ...
            $this->_elements[] = $this->_mform->createElement('select', $key, get_string($key, 'form'), $date, $this->getAttributes(), true);
        }
        if (right_to_left()) {   // Switch order of elements for Right-to-Left
            $this->_elements[] = $this->_mform->createElement('select', 'minute', get_string('minute', 'form'), $minutes, $this->getAttributes(), true);
            $this->_elements[] = $this->_mform->createElement('select', 'hour', get_string('hour', 'form'), $hours, $this->getAttributes(), true);
        } else {
            $this->_elements[] = $this->_mform->createElement('select', 'hour', get_string('hour', 'form'), $hours, $this->getAttributes(), true);
            $this->_elements[] = $this->_mform->createElement('select', 'minute', get_string('minute', 'form'), $minutes, $this->getAttributes(), true);
        }
        ...
        if ($calendartype->get_name() === 'gregorian') {
            ...
            $this->_elements[] = $this->_mform->createElement('link', 'calendar',
                    null, '#', $image,
                    array('class' => 'visibleifjs'));
        }
        // If optional we add a checkbox which the user can use to turn if on
        if ($this->_options['optional']) {
            $this->_elements[] = $this->_mform->createElement('checkbox', 'enabled', null, get_string('enable'), $this->getAttributes(), true);
        }
