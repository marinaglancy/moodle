...
class core_text {
    ...
    public static function substr($text, $start, $len=null, $charset='utf-8') {
        ...
        if (self::is_charset_supported($charset)) {
            $result = mb_substr((string)$text, $start, $len, $charset);
        } else {
            $result = (string)iconv_substr((string)$text, $start, $len, $charset);
        }
    ...
    public static function str_max_bytes($string, $bytes) {
        return mb_strcut((string)$string, 0, $bytes, 'UTF-8');
    }
    ...
    public static function strlen($text, $charset='utf-8') {
        ...
        if (self::is_charset_supported($charset)) {
            return mb_strlen((string)$text, $charset);
        }

        return iconv_strlen((string)$text, $charset);
    }
    ...
    public static function strtolower($text, $charset='utf-8') {
        ...
        if (self::is_charset_supported($charset)) {
            return mb_strtolower((string)$text, $charset);
        }
    ...
    public static function strtoupper($text, $charset='utf-8') {
        ...
        // Confirm mbstring can handle the charset.
        if (self::is_charset_supported($charset)) {
            return mb_strtoupper((string)$text, $charset);
        }
    ...
    public static function strpos($haystack, $needle, $offset=0) {
        return mb_strpos((string)$haystack, $needle, $offset, 'UTF-8');
    }
    ...
    public static function utf8_to_entities($str, $dec=false, $nonnum=false) {
        ...
        $result = mb_strtolower(mb_encode_numericentity((string)$str, [0xa0, 0xffff, 0, 0xffff], 'UTF-8', true));
