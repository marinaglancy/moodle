...
function s($var) {
    ...
    return preg_replace('/&amp;#(\d+|x[0-9a-f]+);/i', '&#$1;',
            htmlspecialchars((string)$var, ENT_QUOTES | ENT_HTML401 | ENT_SUBSTITUTE));
}
...
function validate_email($address) {
    ...
    return moodle_phpmailer::validateAddress((string)$address) && !preg_match('/[<>]/', $address);
}
...
function replace_ampersands_not_followed_by_entity($string) {
    return preg_replace("/\&(?![a-zA-Z0-9#]{1,8};)/", "&amp;", (string)$string);
}
