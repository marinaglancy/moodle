...
class admin_setting_manageformats extends admin_setting {
    ...
    public function __construct() {
        $this->nosave = true;
        parent::__construct('formatsui', get_string('manageformats', 'admin'), '', '');
    }
    ...
    public function output_html($data, $query='') {
        global $CFG, $OUTPUT;
        $return = '';
        $return = $OUTPUT->heading(get_string('courseformats'), 3, 'main');
        ...
        foreach ($formats as $format) {
            ...
            if ($cnt) {
                $updown .= html_writer::link($url->out(false, array('action' => 'up')),
                    $OUTPUT->pix_icon('t/up', $txt->up, 'moodle', array('class' => 'icon')));
            }
            if ($cnt < count($formats) - 1) {
                $updown .= html_writer::link($url->out(false, array('action' => 'down')),
                    $OUTPUT->pix_icon('t/down', $txt->down, 'moodle', array('class' => 'icon')));
            }*/
            $cnt++;
            $settings = '';
            ...
            $table->data[] =array($strformatname/*, $hideshow, $updown*/, $settings);
        }
        $return .= html_writer::table($table);
        $return .= $OUTPUT->box_end();
        return highlight($query, $return);
