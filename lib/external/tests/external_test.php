...
class core_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_update_inplace_editable() {
        ...
        // Call service for component that does not have inplace_editable_saver class.
        try {
            core_external::update_inplace_editable('some_component', 'itemtype', 1, 'newvalue');
            $this->fail('Exception expected');
        } catch (moodle_exception $e) {
            $this->assertEquals('Coding error detected, it must be fixed by a programmer: Class '
                    . 'some_component\inplace_editable_saver not found or is not allowed here',
                    $e->getMessage());
        }
