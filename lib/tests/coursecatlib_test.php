...
class coursecatlib_testcase extends advanced_testcase {
    ...
    public function test_get_search_courses() {
        ...
        // search courses (only global search is implemented)
        $cat0 = coursecat::get(0);

        // search by text
        $res = $cat0->search_courses(array('search' => 'test'), array('recursive' => 1));
        $this->assertEquals(array($c4->id, $c3->id, $c1->id, $c8->id, $c5->id), array_keys($res));
        $this->assertEquals(5, $cat0->search_courses_count(array('search' => 'test'), array()));

        $res = $cat0->search_courses(array('search' => 'Математика'), array('recursive' => 1));
        $this->assertEquals(array($c3->id, $c6->id), array_keys($res));
        $this->assertEquals(2, $cat0->search_courses_count(array('search' => 'Математика'), array()));

        $options = array('recursive' => 1, 'sort' => array('fullname' => 1), 'offset' => 1, 'limit' => 2);
        $res = $cat0->search_courses(array('search' => 'test'), $options);
        $this->assertEquals(array($c4->id, $c5->id), array_keys($res));
        $this->assertEquals(5, $cat0->search_courses_count(array('search' => 'test'), $options));
    }
}