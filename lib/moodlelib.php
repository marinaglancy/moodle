...
function calculate_user_dst_table($fromyear = null, $toyear = null, $strtimezone = null) {
    ...
    if (!isset($presetscache[$usertz])) {
        $presetscache[$usertz] = $DB->get_records('timezone', array('name' => $usertz), 'year DESC', 'year, gmtoff, dstoff, dst_month, dst_startday, dst_weekday, dst_skipweeks, dst_time, std_month, ' .
 ...
 * @throws require_login_exception
 */
function validate_login($logintype, $course, $cm) {
...
function remove_course_contents($courseid, $showfeedback = true, array $options = null) {
    ...
    $DB->delete_records_select('course_modules_avail_fields', 'coursemoduleid IN (SELECT id from {course_modules} WHERE course=?)', array($courseid));
    ...
    $DB->delete_records_select('course_sections_avail_fields', 'coursesectionid IN (SELECT id from {course_sections} WHERE course=?)', array($course->id));
