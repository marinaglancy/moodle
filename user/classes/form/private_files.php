 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class private_files extends \core_form\modal {
    ...
    public function definition() {
        ...
        if ($maxareabytes != FILE_AREA_MAX_BYTES_UNLIMITED) {
            $fileareainfo = file_get_file_area_info($this->get_context_for_ajax_submission()->id, 'user', 'private');
    ...
    protected function check_access_for_ajax_submission() {
        require_capability('moodle/user:manageownfiles', $this->get_context_for_ajax_submission());
    }
    ...
    protected function get_options(): array {
        ...
        if (has_capability('moodle/user:ignoreuserquota', $this->get_context_for_ajax_submission())) {
    ...
    public function process_ajax_submission() {
        file_postupdate_standard_filemanager($this->get_data(), 'files',
            $this->get_options(), $this->get_context_for_ajax_submission(), 'user', 'private', 0);
        return null;
    }
    ...
    public function set_data_for_ajax_submission() {
        ...
        file_prepare_standard_filemanager($data, 'files', $this->get_options(),
            $this->get_context_for_ajax_submission(), 'user', 'private', 0);
        $this->set_data($data);
    }
