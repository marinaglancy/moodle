    ...
    public function update_registration($registeredhub) {
        ...
        if (extension_loaded('xmlrpc')) {
            ...
            try {
                ...
                $this->update_registeredhub($registeredhub); // To update timemodified.
                return true;
            } catch (Exception $e) {
                $errormessage = $e->getMessage();
            }
        } else if ($registeredhub->huburl == HUB_MOODLEORGHUBURL) {
            // If php_xmlrpc is not installed moodle.net provides a fallback.
            $siteinfo['token'] = $registeredhub->token;
            $siteinfo['json'] = 1;
            $curl = new curl();
            $curloutput = @json_decode($curl->get($registeredhub->huburl . '/local/hub/siteregistration.php', $siteinfo));
            $info = $curl->get_info();
            if ($curl->get_errno()) {
                $errormessage = $curl->error;
            } else if (isset($curloutput->error)) {
                $errormessage = $curloutput->error;
        ...
        $errorparam->errormessage = $errormessage;
        $errorparam->hubname = $registeredhub->hubname;
        throw new moodle_exception('errorcron', 'hub', '', $errorparam);
