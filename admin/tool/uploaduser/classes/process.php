            ...
            if (!empty($user->$column)) {
                ...
                if (is_object($this->cohorts[$addcohort])) {
                    ...
                    if (!$DB->record_exists('cohort_members', ['cohortid' => $cohort->id, 'userid' => $user->id])) {
                        ...
                        $this->upt->track('enrolments', get_string('useradded', 'core_cohort', s($cohort->name)));
                    }
                } else {
                    // Error message.
        ...
        foreach ($this->get_file_columns() as $column) {
                    ...
                    if ($removing) {
                        if (user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
                            ...
                            $this->upt->track('enrolments', get_string('unassignedsysrole',
                                'tool_uploaduser', $this->sysrolecache[$sysroleid]->name));
                        }
                    } else {
                        if (!user_has_role_assignment($user->id, $sysroleid, SYSCONTEXTID)) {
            ...
            if ($courseid == SITEID) {
                ...
                if (!empty($user->{'role'.$i})) {
                    ...
                    $this->upt->track('enrolments', get_string('enrolledincourserole', 'enrol_manual', $a));
                }
            ...
            if (!empty($user->{'group'.$i})) {
                ...
                try {
                    if (groups_add_member($gid, $user->id)) {
                        $this->upt->track('enrolments', get_string('addedtogroup', '', s($gname)));
                    } else {
