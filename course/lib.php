...
//  extra - contains extra string to include in any link
    ...
    if ($sections = $DB->get_records('course_sections', array('course' => $courseid), 'section ASC', 'id,section,sequence,visible')) {
        ...
        if ($errormessages = course_integrity_check($courseid, $rawmods, $sections)) {
            ...
            $rawmods = get_course_mods($courseid);
            $sections = $DB->get_records('course_sections', array('course' => $courseid), 'section ASC', 'id,section,sequence,visible');
        }
...
function set_coursemodule_visible($id, $visible, $visibleoncoursepage = 1) {
    ...
    if (($cm->visible != $visible) && ($events =
            $DB->get_records('event', array('instance' => $cm->instance, 'modulename' => $modulename)))) {
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    if (!isset($str)) {
        $str = get_strings(array('delete', 'move', 'moveright', 'moveleft',
            'editsettings', 'duplicate', 'hide', 'hideoncoursepage', 'show'), 'moodle');
        ...
        if ($format === 'site') {
            $str->hideoncoursepage = get_string('hideonfrontpage');
        } else if (get_string_manager()->string_exists('hideoncoursepage', 'format_' . $format)) {
            $str->hideoncoursepage = get_string('hideoncoursepage', 'format_' . $format);
        }
    }
    ...
    if (has_capability('moodle/course:activityvisibility', $modcontext)) {
        if ($mod->visible) {
            $allowstealth = $courseformat->allow_stealth_module_visibility($mod, $mod->get_section_info());
            if ($mod->visibleoncoursepage) {
                $class = 'editing_show';
                $actionimage = 't/hide';
                if ($allowstealth) {
                    $actionname = 'hideoncoursepage';
                    $title = $str->hideoncoursepage;
                } else {
                    $title = $str->hide;
                    $actionname = 'hide';
                }
                ...
                $title = $str->hide;
                $actionname = 'hide';
                $class = 'editing_hideoncoursepage';
                $actionimage = 't/hideoncoursepage';
            }
        } else {
            $actionname = 'show';
            $actionimage = 't/show';
        ...
        $actions[$actionname] = new action_menu_link_secondary(
            new moodle_url($baseurl, array($actionname => $mod->id)),
            new pix_icon($actionimage, $title, 'moodle', array('class' => 'iconsmall', 'title' => '')),
            $title,
            array('class' => $class, 'data-action' => $actionname, 'data-sr' => $sr)
        );
    }
        ...
        if (plugin_supports('mod', $mod->modname, FEATURE_GROUPS, 0)) {
            ...
            $actions[$actionname] = new action_menu_link_primary(
                ...
                array('class' => 'editing_'. $actionname, 'data-action' => $nextactionname,
                    'aria-live' => 'assertive', 'data-sr' => $sr)
            );
        } else {
    ...
    if (has_capability('moodle/role:assign', $modcontext)){
        $actions['assign'] = new action_menu_link_secondary(
            ...
            $str->assign,
            array('class' => 'editing_assign', 'data-action' => 'assignroles', 'data-sr' => $sr)
        );
    }
    ...
    if ($hasmanageactivities) {
        $actions['delete'] = new action_menu_link_secondary(
            ...
            $str->delete,
            array('class' => 'editing_delete', 'data-action' => 'delete', 'data-sr' => $sr)
        );
    }
...
function course_get_cm_move(cm_info $mod, $sr = null) {
    ...
    if ($hasmanageactivities) {
        ...
        return html_writer::link(
            ...
            array('class' => 'editing_move', 'data-action' => 'move', 'data-sr' => $sr)
        );
    }
    return '';
}
...
function include_course_ajax($course, $usedmodules = array(), $enabledmodules = null, $config = null) {
    ...
    $PAGE->requires->js_call_amd('core/course', 'init_course_page', array($course->format));
...
function core_course_inplace_editable($itemtype, $itemid, $newvalue) {
    ...
    if ($itemtype === 'activityname') {
        ...
        $courserenderer = $PAGE->get_renderer('core', 'course');
        return $courserenderer->course_section_cm_name_tmpl($cm, true);
    }
}
