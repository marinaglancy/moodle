...
abstract class format_base {
    ...
    /**
     * Returns course section and creates new if does not exist yet
     *
     * @param int $sectionnum section number
     * @return section_info
     */
    function get_or_create_section($sectionnum) {
        global $DB;
        if ($section = $this->get_section($sectionnum)) {
            return $section;
        }
        $cw = new stdClass();
        $cw->course   = $this->courseid;
        ...
        // rebuild_course_cache() will destroy this instance, so we make sure we create another one
        return course_get_format($this->get_course())->get_section($sectionnum);
    }
    ...
    /**
     * Section name to display on overview pages
     *
     * For course formats with nested sections this can return the full section name
     * with all parents
     *
     * @param type $section
     * @return type 
     */
    public function get_section_full_name($section) {
    ...
    /**
     * Returns url for viewing this section or null if this section can not be viewed
     * on a separate page
     *
     * @param int|stdClass $section Section object from database or just field section.section
    ...
    public function get_section_url($section) {
        if (($course = $this->get_course()) && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            ...
            return course_get_url($course, $sectionnum);
        }
        return null;
    }
...
class format_legacy extends format_base {
    ...
    function get_section_url($section) {
        ...
        if (function_exists($featurefunction) && ($course = $this->get_course())) {
            ...
            return $featurefunction($course, $sectionnum);
        }

        // else, default behavior:
        return parent::get_section_url($section);        
    }
