...
namespace auth_db\local\hooks\navigation;

use admin_setting_configpasswordunmask;
use admin_setting_configselect;
use admin_setting_configtext;
use admin_setting_heading;
use auth_db_admin_setting_special_auth_configtext;
use lang_string;

/**
 * Hook callbacks for auth_db
 *
 * @package    auth_db
 * @copyright  2023 Marina Glancy
 * @author     2017 Stephen Bourget
    ...
    public static function callback(\core\hook\navigation\site_administration_extend $hook): void {
        ...
        if ($settings && $ADMIN->fulltree) {
            ...
            $settings->add(new admin_setting_configselect('auth_db/sybasequoting',
                new lang_string('auth_dbsybasequoting', 'auth_db'), new lang_string('auth_dbsybasequotinghelp', 'auth_db'), 0,
                $yesno));
            ...
            $settings->add(new admin_setting_configselect('auth_db/passtype',
                new lang_string('auth_dbpasstype_key', 'auth_db'), new lang_string('auth_dbpasstype', 'auth_db'), 'plaintext',
                $passtype));
