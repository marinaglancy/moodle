...
/**
 * Class to represent an item in mod_chat recent activity.
 *
 * @package    mod_chat
...
class recentactivity extends \core_course\recentactivity {
    ...
    public static function get_recentactivity_types($forblock = false) {
        if ($forblock) {
            return array(
                'chat_past' => new \lang_string('pastchats', 'chat'),
                'chat_current' => new \lang_string('currentchats', 'chat')
            );
        } else {
            return array();
        }
    ...
    public static function get_recentactivity($course, $timestart, $filters = array(), $forblock = false) {
        ...
        $activities = array();
        if (!$forblock) {
            // Chat sessions are only displayed in a block.
            return $activities;
        }
        return $activities;
    }
