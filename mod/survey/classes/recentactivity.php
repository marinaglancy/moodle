...
/**
 * Class to represent an item in mod_survey recent activity.
 *
 * @package    mod_survey
...
class recentactivity extends \core_course\recentactivity {
    ...
    public static function get_recentactivity_types($forblock = false) {
        if ($forblock) {
            return array(
                'survey' => new \lang_string('newsurveyresponses', 'survey')
            );
        } else {
            return array();
        }
    ...
    public static function get_recentactivity($course, $timestart, $filters = array(), $forblock = false) {
        ...
        $activities = array();
        if (!$forblock) {
            // Survey responses are only displayed in a block.
            return $activities;
        }
        return $activities;
    }
