...
// Checking login and permissions.
...
list($contextmodule, $course, $cm) = $PAGE->login_to_cm('scorm', $cmid);
require_capability('mod/scorm:viewreport', $contextmodule);
$scormid = $cm->instance;

// Building the url to use for links.
$url = new moodle_url('/mod/scorm/report/userreporttracks.php', array('id' => $cmid,
    'user' => $userid,
    'attempt' => $attempt,
    'scoid' => $scoid));
$PAGE->set_url($url);
...
$event = \mod_scorm\event\tracks_viewed::create(array(
    ...
    'other' => array('attemptid' => $attempt, 'instanceid' => $scormid, 'scoid' => $scoid)
));
...
$PAGE->navbar->add("$strattempt $attempt - ".fullname($user),
    new moodle_url('/mod/scorm/report/userreport.php', array('id' => $cmid, 'user' => $userid, 'attempt' => $attempt)));
...
$exportfilename = $courseshortname . '-' . $cm->get_formatted_name() . '-' . get_string('details', 'scorm');
