...
class mod_workshop_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        foreach (['submissiontypetext', 'submissiontypefile'] as $type) {
            ...
            // Add a line break so each pair is visually on a line together.
            $typeelements[] = $mform->createElement('static', 'divider', '', html_writer::empty_tag('br'));
            $mform->setDefault($available, 1);
            $mform->disabledIf($required, $available);
        }
        $mform->setDefault('submissiontypefilerequired', 1);
        $mform->addGroup($typeelements, 'submissiontypes', get_string('submissiontypes', 'workshop'), ' ', false);
        ...
        if ($this->current->instance) {
            ...
            foreach (['submissiontypetext', 'submissiontypefile'] as $type) {
                ...
                if ($data[$type] != WORKSHOP_SUBMISSION_TYPE_DISABLED) {
                    ...
                    if ($data[$type] == WORKSHOP_SUBMISSION_TYPE_REQUIRED) {
    ...
    public function validation($data, $files) {
        ...
        } else if (!($data['submissiontypetextavailable'] && !empty($data['submissiontypetextrequired']))
                && !($data['submissiontypefileavailable'] && !empty($data['submissiontypefilerequired']))) {
            // One available submission type must be  required (otherwise students can submit blank submissions).
            $errors['submissiontypes'] = get_string('nosubmissiontyperequired', 'workshop');
        }
