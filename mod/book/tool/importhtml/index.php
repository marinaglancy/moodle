...
$PAGE->set_url('/mod/book/tool/importhtml/index.php', array('id'=>$cmid, 'chapterid'=>$chapterid));
...
$mform = new booktool_importhtml_form(null, array('id'=>$cmid, 'chapterid'=>$chapterid));

// If data submitted, then process and store.
if ($mform->is_cancelled()) {
    if (empty($chapter->id)) {
        redirect($CFG->wwwroot."/mod/book/view.php?id=$cmid");
    } else {
        redirect($CFG->wwwroot."/mod/book/view.php?id=$cmid&chapterid=$chapter->id");
    }
    ...
    echo $OUTPUT->continue_button(new moodle_url('/mod/book/view.php', array('id'=>$cmid)));
    echo $OUTPUT->footer();
    die;
}
