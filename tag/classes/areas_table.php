    ...
    public function __construct() {
        ...
        foreach ($tagareas as $itemtype => $it) {
            foreach ($it as $component => $record) {
                $areaname = core_tag_area::display_name($record->itemtype, $record->component);
                $baseurl = new moodle_url('/tag/collections.php', array('ta' => $record->id, 'sesskey' => sesskey()));
                if ($record->enabled) {
