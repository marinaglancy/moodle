...
class core_tag_area {
     ...
     * @return lang_string
     */
    public static function display_name($itemtype, $component) {
     ...
     * @return int
     */
    public static function get_collection($itemtype, $component) {
        $itemtypes = core_tag_area::get_areas();
    ...
    protected static function delete($record) {
        ...
        core_tag::delete_instances($record->component, $record->itemtype);
    ...
    protected static function create($record) {
        ...
        $DB->insert_record('tag_area', array('component' => $record->component,
            'itemtype' => $record->itemtype, 'tagcollid' => $record->tagcollid));
    ...
    public static function update($existing, $data) {
        ...
        if (!empty($data['tagcollid'])) {
            self::move_tags($existing->itemtype, $existing->component, $data['tagcollid']);
        }
    ...
    public static function reset_definitions_for_component($componentname) {
        ...
        if ($tagareas) {
            foreach ($tagareas as $tagarea) {
                ...
                if ($component !== 'core' || !isset($record->component)) {
                ...
                if (!empty($record->collection)) {
                    ...
                    if (!isset($existingcolls[$key])) {
                        $tagcoll = (object)array('name' => $record->collection,
                                    'component' => $record->component);
                        $tagcoll->id = core_tag_collection::create($tagcoll);
                        $existingcolls[$key] = $tagcoll;
                    }
        ...
        foreach ($toupdatearea as $key => $tagarea) {
            if (!isset($itemtypes[$key]->tagcollid)) {
                foreach ($todeletecoll as $tagcoll) {
                    if ($tagcoll->id == $tagarea->id) {
                        $tagarea->id = core_tag_collection::get_default();
                    }
                }
            }
    ...
    public static function uninstall($pluginname) {
        ...
        core_tag::delete_instances($component);
     ...
     * @param int $tagcollid
     */
    public static function move_tags($itemtype, $component, $tagcollid) {
        global $DB;
        ...
        if ($relatedtags) {
            $tags = core_tag::get_by_name_bulk($tagcollid, array_keys($relatedtags));
            foreach ($tags as $tag) {
