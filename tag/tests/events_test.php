...
class core_tag_events_testcase extends advanced_testcase {
    ...
    public function test_tag_deleted() {
        ...
        core_tag::add_item_tag('course', 'core', $course->id, context_course::instance($course->id), 'cat', $user->id);

        // Trigger and capture the event for deleting a personal tag for a user for a course.
        $sink = $this->redirectEvents();
        core_tag::remove_item_tag('course', 'core', $course->id, 'cat', $user->id);
        $events = $sink->get_events();
        $event = $events[1];
        ...
        core_tag::add_item_tag('course', 'core', $course->id, context_course::instance($course->id), 'dog', $user->id);

        // Trigger and capture the event for deleting all tags in a course.
        $sink = $this->redirectEvents();
        core_tag::remove_all_item_tags('course', 'core', $course->id);
        $events = $sink->get_events();
        $event = $events[1];
        ...
        core_tag::set_item_tags('course', 'core', $course->id, context_course::instance($course->id),
    ...
    public function test_tag_created() {
        ...
        $sink = $this->redirectEvents();
        core_tag::create_if_missing(core_tag_area::get_collection('course', 'core'),
