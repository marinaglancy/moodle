...
class core_tag_taglib_testcase extends advanced_testcase {
     ...
     * Test the tag_set_delete function.
     */
    public function test_tag_remove() {
        global $DB;
        ...
        // Create the tag and tag instance we are going to delete.
        core_tag::add_item_tag('course', 'core', $course->id, context_course::instance($course->id), 'A random tag');

        // Call the tag_set_delete function.
        core_tag::remove_item_tag('course', 'core', $course->id, 'A random tag');
    ...
    public function test_cleanup() {
        ...
        $dogtag = core_tag::get_by_name($defaultcoll, 'dog');
        $fishtag = core_tag::get_by_name($defaultcoll, 'fish');
        ...
        tag_set('user', $user->id, array('sometag'),
                'core', context_user::instance($user->id)->id);
        $sometag = core_tag::get_by_name($defaultcoll, 'sometag');
    ...
    public function test_guess_tag() {
        ...
        $tcid = core_tag_collection::create((object)array('name' => 'tagcoll'));
        $tag2 = $this->getDataGenerator()->create_tag(array('name' => 'Cat', 'tagcollid' => $tcid));
        $this->assertEquals(2, count($DB->get_records('tag')));
        $this->assertEquals(2, count(core_tag::guess_by_name('Cat')));
        $this->assertEquals(core_tag_collection::get_default(), core_tag::get_by_name(0, 'Cat')->tagcollid);
    }
    ...
    public function test_instances() {
        ...
        core_tag::set_item_tags('course', 'core', $course->id, $context, array('Tag 1', 'Tag 2'));
        $tags = core_tag::get_item_tags('course', 'core', $course->id);
        ...
        core_tag::set_item_tags('course', 'core', $course->id, $context, array('Tag 3', 'Tag 2', 'Tag 1'));
        $tags = core_tag::get_item_tags('course', 'core', $course->id);
        $tagssimple = array_values($tags);
        ...
        core_tag::set_item_tags('course', 'core', $course->id, $context, array('Tag 3'));
        $tags = core_tag::get_item_tags('course', 'core', $course->id);
        $tagssimple = array_values($tags);
    ...
    public function test_related_tags() {
        ...
        $tag = core_tag::get($tag->id, '*');

        $tag->set_related_tags(array('Synonym 1', 'Synonym 2'));
        $relatedtags = array_values(core_tag::get_item_tags('tag', 'core', $tag->id));
        ...
        $t1 = core_tag::get_by_name($tagcollid, 'Synonym 1', '*');
        $relatedtags = array_values(core_tag::get_item_tags('tag', 'core', $t1->id));
        $this->assertEquals(1, count($relatedtags));
        $this->assertEquals('My tag', $relatedtags[0]->rawname);

        $t2 = core_tag::get_by_name($tagcollid, 'Synonym 2', '*');
        $relatedtags = array_values(core_tag::get_item_tags('tag', 'core', $t2->id));
        $this->assertEquals(1, count($relatedtags));
        $this->assertEquals('My tag', $relatedtags[0]->rawname);
        ...
        $t3 = core_tag::get_by_name($tagcollid, 'Synonym 3', '*');
        $relatedtags = array_values(core_tag::get_item_tags('tag', 'core', $t3->id));
        $this->assertEquals(1, count($relatedtags));
        $this->assertEquals('My tag', $relatedtags[0]->rawname);
    ...
    public function test_tag_coll_basic() {
        ...
        $tagcollid1 = core_tag_collection::create($data);
        $tagcolls = core_tag_collection::get_collections();
        ...
        $tagcollid2 = core_tag_collection::create($data);
        $tagcolls = core_tag_collection::get_collections();
    ...
    protected function prepare_move_tags() {
        ...
        $collid2 = core_tag_collection::create(array('name' => 'newcoll'));
        $user1 = $this->getDataGenerator()->create_user();
        $user2 = $this->getDataGenerator()->create_user();
        ...
        core_tag::set_item_tags('user', 'core', $user1->id, context_user::instance($user1->id),
                array('Tag1', 'Tag2'));
        core_tag::set_item_tags('user', 'core', $user2->id, context_user::instance($user2->id),
                array('Tag2', 'Tag3'));
    ...
    public function test_move_tags_simple() {
        ...
        $this->assertEquals(array('Tag1', 'Tag2'), array_values(core_tag::get_item_tags_array('user', 'core', $user1->id)));
        $this->assertEquals(array('Tag2', 'Tag3'), array_values(core_tag::get_item_tags_array('user', 'core', $user2->id)));
    }
    ...
    public function test_move_tags_split_tag() {
        ...
        core_tag::set_item_tags('post', 'core', $blogpost->id, context_system::instance(),
                array('Tag1', 'Tag3'));
        ...
        $this->assertEquals(array('Tag1', 'Tag2'), array_values(core_tag::get_item_tags_array('user', 'core', $user1->id)));
        $this->assertEquals(array('Tag2', 'Tag3'), array_values(core_tag::get_item_tags_array('user', 'core', $user2->id)));
        $this->assertEquals(array('Tag1', 'Tag3'), array_values(core_tag::get_item_tags_array('post', 'core', $blogpost->id)));
    }
    ...
    public function test_move_tags_merge_tag() {
        ...
        // Move 'user' area from collection 1 to collection 2, make sure tag Tag2 was moved and tags Tag1 and Tag3 were merged into existing.
        $tagareauser = $DB->get_record('tag_area', array('itemtype' => 'user', 'component' => 'core'));
        core_tag_area::update($tagareauser, array('tagcollid' => $collid2));
    ...
    public function test_move_tags_with_related() {
        ...
        core_tag::get_by_name($collid1, 'Tag1')->set_related_tags(array('Tag2', 'Tag4'));
        ...
        $this->assertEquals(array('TAG1', 'Tag2'), array_values(core_tag::get_item_tags_array('user', 'core', $user1->id)));
        $this->assertEquals(array('Tag2', 'Tag3'), array_values(core_tag::get_item_tags_array('user', 'core', $user2->id)));
        $tag11 = core_tag::get_by_name($collid1, 'Tag1');
        $related11 = tag_get_related_tags_csv(tag_get_related_tags($tag11->id, TAG_RELATED_MANUAL), TAG_RETURN_TEXT);
        $this->assertEquals('Tag2, Tag4', $related11);
        $tag21 = core_tag::get_by_name($collid2, 'TAG1');
        $related21 = tag_get_related_tags_csv(tag_get_related_tags($tag21->id, TAG_RELATED_MANUAL), TAG_RETURN_TEXT);
        $this->assertEquals('Tag2, Tag4', $related21);
    }
}
