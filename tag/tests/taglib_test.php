...
class core_tag_taglib_testcase extends advanced_testcase {
    ...
    public function test_correlations() {
        ...
        $this->assertEquals(1, count($correlatedtags));
        $correlatedtag = reset($correlatedtags);
        $this->assertEquals('cats', $correlatedtag->rawname);

        // Make sure tag_get_correlated() returns 'dogs' and 'puppy' as the correlated tags to the 'dog'.
        $correlatedtags = tag_get_correlated($tags['dog']);
        $this->assertEquals(2, count($correlatedtags));
        $correlatedtag = reset($correlatedtags);
        $this->assertEquals('dogs', $correlatedtag->rawname);
        $correlatedtag = next($correlatedtags);
        $this->assertEquals('puppy', $correlatedtag->rawname);

        // Function tag_get_related_tags() will return both related and correlated tags.
        $relatedtags = tag_get_related_tags($tags['cat']);
        $this->assertEquals(2, count($relatedtags));
        $relatedtag = reset($relatedtags);
        $this->assertEquals('kitten', $relatedtag->rawname);
        $relatedtag = next($relatedtags);
        $this->assertEquals('cats', $relatedtag->rawname);
        ...
        // Make sure tag_get_correlated() and core_tag::get_item_tags() return the same set of fields.
