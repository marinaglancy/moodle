...
abstract class repository implements cacheable_object {
    ...
    protected function sync_reference_with_url($file, $url,
        ...
        $getbody = $storecontent || ($file->get_contenthash() !== sha1(''));
        if ($getbody) {
            $saveas = $this->prepare_file('');
            $curloptions['filepath'] = $saveas;
            $bodydownloaded = $curl->download_one($url, array(), $curloptions) === true;
        } else {
            $curl->head($url, $curloptions);
            $bodydownloaded = false;
        }
        $info = $curl->get_info();
        if (isset($info['http_code']) && $info['http_code'] == 200) {
            if ($bodydownloaded && $storecontent) {
